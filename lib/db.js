...
function normalizeConfig(cfg) {
  ...
  // minFloorPrice: must be <= standard rate; default 70% of standard
  const floor = cfg.minFloorPrice || Math.round(std * 0.7);
  return {
    ...cfg,
    ...
    suiteRate:     suite,
    minFloorPrice:       floor,
    // Hotel identity & display fields
    emoji:               cfg.emoji               || "🏨",
    address:             cfg.address             || "",
    addressLine:         cfg.addressLine         || cfg.address || "",
    distanceTag:         cfg.distanceTag         || "",
    amenities:           Array.isArray(cfg.amenities) ? cfg.amenities : (cfg.amenities ? cfg.amenities.split(",").map(s=>s.trim()).filter(Boolean) : []),
...
export function saveHotelConfig(hotelId, data) {
  ...
  const floor = data.minFloorPrice || Math.round(std * 0.7);
  const amenitiesArr = Array.isArray(data.amenities)
    ? data.amenities
  ...
  if (sb) {
    sb.from("hotels").update({
      ...
      total_rooms:          normalized.totalRooms        || 20,
      emoji:                normalized.emoji             || "🏨",
      address_line:         normalized.addressLine       || "",
      distance_tag:         normalized.distanceTag       || "",
      amenities:            amenitiesArr,
      standard_rate:        std,
      deluxe_rate:          dlx,
      suite_rate:           suite,
      ...
      checkin_time:         normalized.checkinTime         || "12:00",
...
async function ensureHotelInSupabase(hid) {
  ...
  try {
    ...
    else console.log("[DB] Hotel auto-synced to Supabase ✓", hid);
  } catch (e) {
...
export async function checkoutBooking(hotelId, bookingId) {
  ...
  if (sb) {
    sb.from("bookings")
      ...
      .eq("id", bookingId)
      .then(({ error }) => {
        if (error) console.warn("[DB] Checkout Supabase sync failed:", error.message);
