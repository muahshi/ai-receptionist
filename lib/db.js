...
export const getHotelConfig = (hotelId) => {
  ...
  // Try hotel-specific config first
  const specific = read(K(hid, "config"), null);
  if (specific) return specific;
  // Fall back to demo hotel list
  const DEMO = getDemoHotels();
  const demo = DEMO.find(h => h.id === hid);
  if (demo) return { ...demo, currency: "₹", gstPercent: 12 };
