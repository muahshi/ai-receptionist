...
function sbRowToBooking(r) {
  // Parse extraGuests — JSON string from Supabase or array from localStorage
  let extraGuests = null;
  if (r.extra_guests) {
    try { extraGuests = typeof r.extra_guests==="string" ? JSON.parse(r.extra_guests) : r.extra_guests; } catch {}
  } else if (r.extraGuests) {
    extraGuests = Array.isArray(r.extraGuests) ? r.extraGuests : null;
  }
  return {
    ...
    extraGuests,
    idImageFront: r.id_image_front || r.idImageFront || null,
    idImageBack:  r.id_image_back  || r.idImageBack  || null,
  };
}
...
export async function createBooking(hotelId, bookingData) {
  ...
  const booking = {
    ...
    extraGuests:  bookingData.extraGuests || null,
    // ID document images (India compliance)
    idImageFront: bookingData.idImageFront || null,
    idImageBack:  bookingData.idImageBack  || null,
  };
  ...
  if (sb) {
    sb.from("bookings").insert({
      ...
      // Multi-guest + ID image support (India compliance)
      total_guests:  booking.totalGuests || 1,
      extra_guests:  booking.extraGuests ? JSON.stringify(booking.extraGuests) : null,
      id_image_front:booking.idImageFront || null,
      id_image_back: booking.idImageBack  || null,
    }).then(({ error }) => {
