...
export function broadcastUpdate(type, hotelId) {
  if (typeof window === "undefined") return;
  try {
    getBroadcast()?.postMessage({ type, hotelId, ts: Date.now() });
    const sentinel = `air_sync_${hotelId}`;
    localStorage.setItem(sentinel, Date.now().toString());
  } catch {}
}
export function onHotelUpdate(cb) {
  ...
  // Also listen for localStorage-based sync
  const storageHandler = (e) => {
    if (e.key?.startsWith("air_sync_") || e.key?.includes("bookings") || e.key?.includes("rooms")) {
      cb({ type: "storage", key: e.key });
    }
  };
  ...
  if (sb) {
    (async () => {
      try {
        ...
        if (error) console.warn("[DB] room status sync:", error.message);
      } catch (e) {
        console.warn("[DB] room status sync:", e.message);
      }
    })();
  }
}
...
export async function createBooking(hotelId, bookingData) {
  ...
  const id     = bookingData.id || uid(); // Use provided ID if available (public booking page sends its own bid)
  ...
  // 1. localStorage — instant (skip if already written by booking page saveBooking)
  const existingBookings = getBookingsSync(hid);
  if (!existingBookings.find(b => b.id === id)) {
    lsW(K(hid, "bookings"), [booking, ...existingBookings]);
  }
  ...
  // Broadcast to all tabs (incl. same tab) — Guests + Reports pages react instantly
