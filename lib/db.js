...
export async function createBooking(hotelId, bookingData) {
  ...
  const sb = getSB();
  if (sb) {
    // Fire and forget — errors logged but UI never waits
    sb.from("bookings").insert({
      ...
      .then(({ error }) => {
        if (error) {
          // Conflict (duplicate id) is OK — booking already exists
          if (error.code !== "23505") {
      ...
      .catch(e => console.warn("[DB] Booking insert error:", e.message));
  }
