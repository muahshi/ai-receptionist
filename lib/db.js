    ...
    (async () => {
      try {
        ...
        if (error) {
          ...
          // ── CRITICAL FIX: Also upsert rooms table directly from guest device ──
          // Manager's phone has different localStorage — it will NEVER receive
          // BroadcastChannel messages from the guest's device.
          // Supabase rooms table is the ONLY shared truth between devices.
          // DashboardView polls rooms table every 10s — this is what makes the
          // room grid block change color on the manager/owner's screen.
          const roomStatus = bookingData.isPublicBooking ? "reserved" : "occupied";
          const cfg = getHotelConfig(hid);
          const rateMap = {
            standard: cfg.rates?.standard || cfg.standardRate || 1200,
            deluxe:   cfg.rates?.deluxe   || cfg.deluxeRate   || 2000,
            suite:    cfg.rates?.suite    || cfg.suiteRate    || 3800,
          };
          ...
          await sb.from("rooms").upsert({
            ...
            type:               roomType,
            base_rate:          rateMap[roomType] || 1200,
            status:             roomStatus,
