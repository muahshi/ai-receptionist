...
// Normalize config — ensure BOTH rates.standard AND standardRate exist (backward compat)
// Phase 1: also ensure new Guest Services fields pass through cleanly
function normalizeConfig(cfg) {
  if (!cfg) return cfg;
  // rates.standard → standardRate
  const r = cfg.rates || {};
  ...
  return {
    ...
    deluxeRate:    dlx,
    suiteRate:     suite,
    // ── Phase 1: Guest Services fields (default-safe) ──────────
...
export function saveHotelConfig(hotelId, data) {
  // Keep rates in BOTH formats for compatibility
  const r   = data.rates || {};
...
export function initializeRooms(hotelId, totalRooms = 20) {
  ...
  const rooms  = Array.from({ length: totalRooms }, (_, i) => {
    ...
    return {
      ...
      type, status:"vacant", currentBookingId:null,
...
export function getRooms(hotelId) {
  ...
  // Sync baseRate from latest config
  const cfg    = getHotelConfig(hid);
  const stdRate = cfg.rates?.standard || cfg.standardRate || 1200;
  const dlxRate = cfg.rates?.deluxe   || cfg.deluxeRate   || 2000;
  const suitRate= cfg.rates?.suite    || cfg.suiteRate    || 3800;
...
export function updateRoomStatus(hotelId, roomId, status, bookingId = null, guestName = "") {
  ...
  const rooms = getRooms(hid).map(r => r.id === roomId
    ...
    : r);
  lsW(K(hid, "rooms"), rooms);
  // Supabase sync for room status
  const sb = getSB();
  if (sb) {
    sb.from("rooms").upsert({
      id: roomId, hotel_id: hid, status,
      current_booking_id: bookingId, guest_name: guestName,
